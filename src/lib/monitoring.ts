...
class MonitoringService {
      ...
      if ('memory' in performance) {
        ...
        if (memoryUsage > 0.8) {
          this.createAlert({
            ...
            message: `Uso de memória: ${(memoryUsage * 100).toFixed(1)}%`,
            metadata: {
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        this.createAlert({
          ...
          message: `Verificação ${index + 1} falhou: ${result.reason}`,
  ...
  private checkPerformanceThresholds(metric: Partial<PerformanceMetrics>): void {
    if (metric.pageLoadTime && metric.pageLoadTime > 3000) {
      this.createAlert({
        ...
        message: `Página demorou ${(metric.pageLoadTime / 1000).toFixed(2)}s para carregar`,
  ...
  createAlert(alert: Omit<Alert, 'id' | 'timestamp' | 'resolved'>): void {
    ...
    // Log imediato para alertas críticos
    if (alert.severity === 'critical') {
      console.error('🚨 ALERTA CRÍTICO:', newAlert);
    }
  }
  ...
  private async processBatchAlerts(): Promise<void> {
    const alerts = this.alertQueue.splice(0, 10); // Processar até 10 alertas por vez
  ...
  private async processBatchMetrics(): Promise<void> {
    const metrics = this.metricsBuffer.splice(0, 20); // Processar até 20 métricas por vez
  ...
  async getRecentAlerts(limitValue: number = 50): Promise<Alert[]> {
    try {
      const q = query(
  ...
  async getSystemHealth(): Promise<SystemHealth[]> {
    try {
      ...
      return snapshot.docs.map(doc => ({
        ...doc.data(),
        lastCheck: (doc.data().lastCheck as any)?.toDate() || new Date()
      })) as SystemHealth[];
    } catch (error) {
  ...
  generatePerformanceReport(metrics: PerformanceMetrics[]): {
    ...
    if (summary.avgPageLoadTime > 3000) {
      recommendations.push('Otimizar tempo de carregamento da página (atual: ' + 
...
// HOC para monitoramento automático de componentes
