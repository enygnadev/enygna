...
export default function SistemasPage({ onBack }: SistemasPageProps) {
    ...
    >
      ...
      >
        <div className="row" style={{ gap: "var(--gap-lg)" }}>
          <button onClick={onBack || (() => window.history.back())} className="button button-ghost">
        ...
        {user && userData && (
          ...
          >
            ...
            <div className="row center" style={{ gap: "var(--gap-sm)", flexWrap: "wrap" }}>
              {hasPermission("ponto") && (
                <span className="tag" style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" }}>
                  🕒 Ponto
                </span>
              )}
              {hasPermission("chamados") && (
                <span className="tag" style={{ background: "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)" }}>
                  🎫 Chamados
                </span>
              )}
              {hasPermission("frota") && (
                <span className="tag" style={{ background: "linear-gradient(135deg, #00ff7f 0%, #8a2be2 100%)" }}>
                  🚗 Frota
                </span>
              )}
              {hasPermission("documentos") && (
                <span className="tag" style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" }}>
                  📄 Documentos
                </span>
              )}
              {userData.permissions?.admin && (
                <span className="tag" style={{ background: "linear-gradient(135deg, #ff6b6b 0%, #feca57 100%)" }}>
                  👑 Admin
                </span>
              )}
              {(!userData.permissions || Object.values(userData.permissions).every((p) => !p)) && (
                <span className="tag" style={{ background: "var(--color-error)" }}>
